...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    if(!message.member.hasPermission("ADMINISTRATOR")) {
      return message.channel.send(":x: Yopu should have admin perms to use this command")
    }
    
    const user = message.mentions.members.first() 
    
    if(!user) {
    return message.channel.send(":x: Please mention the person whose warning you want to reset")
    }
    
    if(message.mentions.users.first().bot) {
      return message.channel.send(":x: Bot are not allowed to have warnings")
    }
    
    if(message.author.id === user.id) {
      return message.channel.send(":x: You are not allowed to reset your warnings")
    }
