...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!url) return message.reply(":x: Please provide a project url!");
    if (!validUrl.isUri(url)) {
      return message.channel.send(":x: Please provide a vaild url!");
    }
    ...
    if (checkIfExsists === null) {
      ...
      await UrlsConfig.create({
        ...
        let embed = new MessageEmbed()
          .setTitle(":white_check_mark: Added Succesfully!")
      ...
      let embed = new MessageEmbed()
        .setTitle(
          ":x: Project is already Registered!"
        )
        .setDescription(
          ":x: The project you're trying to register is already in the Database"
        )
        .setColor(colors.uptime)
        .setTimestamp();
